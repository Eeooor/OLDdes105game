using System.Collections;
using System.Collections.Generic;
using UnityEngine;


/*------- Challenge 4 ------  Intermediate ------
     * 1. Use Random.Range and create a random vector3 for a leaf direction of movement
     * 2. Create a float value for the speed of each leaf. 
     * 3. Set the speed for each leaf to a random value between 0f and 0.1f
     * 4. Use transform.position, the direction vector and speed to move the leaf
*/


public class LeafController : MonoBehaviour
{
    Vector3 randomDirection = Vector3.zero;
    float leafSpeed = 0.1f;
    // Start is called before the first frame update
    void Start()
    {

    }

    // Update is called once per frame
    void Update()
    {
        randoMovement();
    }

    void randoMovement() //random movment
    {
        leafSpeed = Random.Range(0f, 0.1f);
        //print("Speed is " + leafSpeed);

        float randoX = Random.Range(0f, 1f);

        float randoY = Random.Range(0f, 1f);

        randomDirection = new Vector3(randoX, randoY, 0);

        randomDirection += transform.position;
        Vector3.Normalize(randomDirection);

        transform.position += (randomDirection * leafSpeed);

    }

}
